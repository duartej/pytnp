...
def getDiff2DPlots( tnpRef, tnp2, *nameOfdataSet ):
	...
	datasetVarList, effName = pytnp.checkbinnedVar( dataSet )
	...
	tableEff1 = pytnp.tableEff( dataSet )
	#tableEff2 = pytnp.tableEff( dataSet2 )
	for teff1 in tableEff1:
		e1,e1lo,e1hi = teff1[effName]
		pt,ptlo,pthi = teff1[ptName]
		eta,etalo,etahi = teff1[etaName]
		e2,e2lo,e2hi = eval('pytnp.getEff( dataSet2,'+ptName+'='+str(pt)+','+etaName+'='+str(eta)+')')
...
#		newErr = sqrt(effErrorOther**2.0+effError**2.0)
		...
		for hist in histoList:
			...
			if k == 0:
				hist['histo'].SetBinContent( b, finalEff )    #WARNINGa
